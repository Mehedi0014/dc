...
if(isset($_POST['submit']) || !empty($_POST)){
    ...
    $qry_img1 = "select * from innerpage_banner where id = '$id'";
   ...
   $sql1 = "UPDATE `innerpage_banner` SET `page_name`= '$page_title', `text`= '$text', `banner_img` = '$page_image' WHERE `id`= '$id'";
            
        $res1 = mysqli_query($con,$sql1) or trigger_error("Query Failed! SQL: $qry_img - Error: " . mysqli_error(), E_USER_ERROR);
    if($res1){
        header("Location: list_banner.php?editmsg=Successfully Updated");
    }else{
        header("Location: list_banner.php?editmsg=Error");
    }
    
   
}else{
    header("Location: list_banner.php?insertmsg=General Error");
}