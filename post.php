                    ...
                    <div class="post-details-catagory">
                        ...
                        <div class="col-md-12">
                            <ul class="list-unstyled">
                                <?php $bloghtml = get_blog_categories($post_key, $con);
                        echo (!empty($bloghtml)) ? $bloghtml : 'Sorry! no blog found';?>
                            </ul>
                        </div>
                    </div>
...
<?php 
      function get_blog_categories($index=null, $conn=null){
                            $blogcatImg = '';
                            $sql_content2 = "SELECT * FROM `blog` WHERE status = '0'";
                            $res_content2 = mysqli_query($conn, $sql_content2) or trigger_error("Query Failed! SQL: $sql_content2 - Error: " . mysqli_error(), E_USER_ERROR);
                            
                            while ($row = mysqli_fetch_assoc($res_content2)){
                                
                                    if(!($row['alias'] == $index)){
                                        $blogcatImg .= '<li class="media" style="padding-bottom:15px !important";>
                                    <img data-wow-delay="0.2s" src="http://disseminare.com/newsite/upload/blog/'. $row['banner_img'].'" alt="" class="img-responsive wow zoomIn animated">
                                    <div class="media-body">
                                        <h1 class="mt-0 mb-1"><a href="http://disseminare.com/post/'.$row['alias'] .'">'.$row['title'] . '</a></h1>
